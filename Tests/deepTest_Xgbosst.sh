...
#
# Generates a file with the set of prediction in the /Predictions/Xgbosst folder
#
# Run: ./Tests/deepTest_Xgbosst.sh
...
if [[ "$HELP" -eq "1" ]]
  then
  echo "                                                                                         "
  echo "Usage: ./deepTest_Xgbosst.sh [options]                                                       "
  echo "                                                                                         "
  ...
  echo "                                                                                         "
  echo "Example: ./deepTest_Xgbosst.sh --test                                                        "
  echo "                                                                                         "
  exit 1
  fi
#
...
function TEST_DATABASE () {
    ...
    readarray -t trainDatabase < Analysis/Xgbosst/Test.csv
    ...
    do
        ...
        PREDICTION=`python3 src/Xgbosst.py Test 0 1 $DNA $AA $GC $L_DNA $L_AA NULL`
        echo "$DOMAIN,$PREDICTION" >> Tests/Predictions/Xgbosst/TestDatabase.txt
    done
}
#
################## CROSS VALIDATION ##################
function VIRAL_CV () {
    ...
    do
        ...
        do
            ...
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Viral >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Viral_CV.txt
            fi
        done
    done
}
...
function BACTERIA_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Bacteria >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Bacteria_CV.txt
            fi
        done
    done
}
...
function ARCHAEA_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Archaea >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Archaea_CV.txt
            fi
        done
    done
}
...
function FUNGI_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Fungi >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Fungi_CV.txt
            fi
        done
    done
}
...
function PLANT_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Plant >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Plant_CV.txt
            fi
        done
    done
}
...
function PROTOZOA_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Protozoa >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Protozoa_CV.txt
            fi
        done
    done
}
...
function MITO_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Mitochondrial >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Mitochondrial_CV.txt
            fi
        done
    done
}
...
function PLASTID_CV () {
    ...
    do
        ...
        do
            if [[ "$seq" -ge "${training_block_min}" ]] && [[ "$seq" -le "${training_block_max}" ]]; then
            echo -e "\033[1;34m[RFSC]\033[0m Run: $seq";
            python3 src/Xgbosst.py CV ${blocks_perc[i]} ${blocks_perc[i+1]} ${DNA[seq]} ${AA[seq]} ${GC[seq]} ${L_DNA[seq]} ${L_AA[seq]} Plastid >> Tests/Predictions/Xgbosst/CrossValidation/Prediction_Plastid_CV.txt
            fi
        done
    done
}
