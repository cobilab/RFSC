...
if [[ "$TYPE" == "NM" ]]; then
    ...
    printf "Identifier\tSize1\tSize2\tSize3\tSize4\tSize7\tLevel1\tLevel2\tLevel3\tLevel4\tLevel7\n" >> Analysis/GeCo/$1/$3
    ...
    do
        if [[ "$runs" == "5" ]]; then
            printf "${array_identifier[$file_res]}\t${array_size[0]}\t${array_size[1]}\t${array_size[2]}\t${array_size[3]}\t${array_size[4]}\t${array_level[0]}\t${array_level[1]}\t${array_level[2]}\t${array_level[3]}\t${array_level[4]}\n" >> Analysis/GeCo/$1/$3
            (( file_res++ ))
            runs=0;
            array_size=()
            array_level=()
